@testset "Chang's equation" begin
    ...
    calc_shearforce(eq, z) = ForwardDiff.derivative(z′ -> calc_moment(eq, z′), z)
    for eq in (ChangEquation(-1, 19; z_0 = 0, F_t = 10, D = 0.6, E, I, k = 3750),
               ChangEquation(-1, 19; z_0 = 0, F_t = 10, D = 0.6, E, I, k = 3750, head_free = false),)
        if eq.M_t == 0
            @test eq.θ_t != 0
        else
            @test eq.θ_t == 0
